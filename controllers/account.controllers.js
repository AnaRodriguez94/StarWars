    ...
    if (!valEmail) {
      ...
      if (valPassword) {
        ...
        return res.status(200).json({
          success: false,
          message: "The password get any error",
        });
      }
    }
    return res
      .status(200)
      .json({ success: false, message: 'You already be registered' });
  } catch (error) {
    console.log(error);
...
const login = async (req, res) => {
  try {
    ...
    if (!userDB) {
      return res.status(200).json({ succe: false, message: "The email is not registered" })
    }
    if (!bcrypt.compareSync(userBody.password, userDB.password)) {
      return res.status(200).json({ succes: true, message: "Wrong password" })
    }
